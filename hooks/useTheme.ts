...
// Hex rengini HSL'e çevirip renk açısını (Hue) 140° kaydırarak uyumlu ve zıt bir renk üretir
function generateComplementaryColor(hex: string): string {
  ...
  // Açı olarak 140 derece döndür (Estetik zıtlık için)
  let targetHue = (h * 360 + 140) % 360;

  // HSL -> RGB dönüşümü (Doygunluk %80, Parlaklık %50 seviyesinde sabit tutulur)
  const s = 0.8;
  const l = 0.5;
  ...
  if (0 <= targetHue && targetHue < 60) { rPrime = c; gPrime = x; bPrime = 0; }
  else if (60 <= targetHue && targetHue < 120) { rPrime = x; gPrime = c; bPrime = 0; }
  else if (120 <= targetHue && targetHue < 180) { rPrime = 0; gPrime = c; bPrime = x; }
  else if (180 <= targetHue && targetHue < 240) { rPrime = 0; gPrime = x; bPrime = c; }
  else if (240 <= targetHue && targetHue < 300) { rPrime = x; gPrime = 0; bPrime = c; }
  else if (300 <= targetHue && targetHue < 360) { rPrime = c; gPrime = 0; bPrime = x; }
...
export function useTheme() {
  ...
  useEffect(() => {
    const savedMode = (localStorage.getItem('backstory-theme-mode') as ThemeMode) || 'dark';
    const savedAccent = localStorage.getItem('backstory-accent-color') || '#f59e0b';
  ...
  const applyTheme = (newMode: ThemeMode, newAccentHex: string) => {
    ...
    root.style.setProperty('--app-accent-foreground', getContrastColor(newAccentHex));
    root.style.setProperty('--app-completed', completedColor);
    root.style.setProperty('--app-completed-foreground', getContrastColor(completedColor));
  };
